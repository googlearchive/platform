...
(function() {
  ...
  */
  ...
  if (true) {
    // patch document.head|body.
    //    appendChild|replaceChild|removeChild|insertBefore
    function makeShadowMethod(inMethod) {
      return function() {
        var wb = wrap(this);
        wb[inMethod].apply(wb, arguments);
      };
    };
    function patchInstance(inNode) {
      var m$ = ['appendChild', 'replaceChild', 'removeChild', 'insertBefore'];
      m$.forEach(function(m) {
        inNode[m] = makeShadowMethod(m);
      });
    }
    patchInstance(document.head);
    document.addEventListener('DOMContentLoaded', function() {
      patchInstance(document.body);
    });
  }
